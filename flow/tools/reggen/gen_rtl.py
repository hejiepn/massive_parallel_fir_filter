...
def gen_rtl(obj, pkg_sv_filename, top_sv_filename):
    # obj: OrderedDict

    block = json_to_reg(obj)

    # Read Register templates
    reg_top_tpl = Template(filename=str(Path(__file__).parent/'reg_top.sv.tpl'))
    reg_pkg_tpl = Template(filename=str(Path(__file__).parent/'reg_pkg.sv.tpl'))

    # Generate pkg.sv with block name
    with open(pkg_sv_filename, 'w', encoding='UTF-8') as fout:
        try:
            fout.write(
