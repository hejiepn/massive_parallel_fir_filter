...
def xvlog(src_files_xvlog, defines, include_dirs, cwd):
    ...
    #subprocess.check_call(["xvlog"]+["-sv"]+xvlog_opts+[str(fn) for fn in sv_sources], cwd=cwd)
    #subprocess.check_call(["xvlog"]+[]+xvlog_opts+[str(fn) for fn in v_sources], cwd=cwd)
