...
static inline void irq_ctrl_disable(void) {
    REG32(IRQ_CTRL_ALL_EN(0)) = 0;
}

static inline void irq_ctrl_enable(void) {
    REG32(IRQ_CTRL_ALL_EN(0)) = 1;
}

static inline void irq_ctrl_set_mask(uint32_t mask_value) {
    REG32(IRQ_CTRL_MASK(0)) = mask_value;
}

static inline void irq_ctrl_set_mask_set(unsigned int n) {
    REG32(IRQ_CTRL_MASK_SET(0)) = 1 << n;
}

static inline void irq_ctrl_set_mask_clear(unsigned int n) {
    REG32(IRQ_CTRL_MASK_CLR(0)) = 1 << n;
}

static inline void irq_ctrl_set_test_mode(bool enable) {
    REG32(IRQ_CTRL_TEST(0)) = enable ? 1 : 0;
}

static inline void irq_ctrl_set_test_irq(uint32_t test_irq_value) {
    REG32(IRQ_CTRL_TEST_IRQ(0)) = test_irq_value;
}

//read functions

static inline unsigned int get_irq_ctrl_all_en(void) {
...
static inline uint32_t get_irq_ctrl_mask(void) {
	return REG32(IRQ_CTRL_MASK(0));
}

static inline uint32_t get_irq_ctrl_status(void) {
    return REG32(IRQ_CTRL_STATUS(0));
}

static inline uint32_t get_irq_ctrl_irq_no(void) {
    return REG32(IRQ_CTRL_IRQ_NO(0));
}

static inline unsigned int get_irq_ctrl_test(void) {
...
static inline uint32_t get_irq_ctrl_test_irq(void) {
